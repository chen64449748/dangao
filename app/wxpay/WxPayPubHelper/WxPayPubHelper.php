...
*/
	...
	include_once("AppWxPay.pub.config.php");
...
class Common_util_pub
{
	 ...
	 * 	作用：生成签名
	 */
	public function getSign($Obj,$out_name='')
	{
		foreach ($Obj as $k => $v)
		{
			$Parameters[$k] = $v;
		...
		if($out_name == 'iOS'){
			$String = $String . "&key=" . AppWxPayConf_pub::$KEY;
		}else{
 ...
 * 关闭订单接口
 */
class CloseOrder_pub extends Wxpay_client_pub
{
	function __construct() {
		//设置接口链接
	...
	function createXml()
	{
		try
		{
			//检测必填参数
			if($this->parameters["out_trade_no"] == null) {
				throw new SDKRuntimeException("取消订单接口中，必须填写参数out_trade_no！"."<br>");
			}
		   	$this->parameters["appid"] = WxPayConf_pub::$APPID;//公众账号ID
		   	$this->parameters["mch_id"] = WxPayConf_pub::$MCHID;//商户号
		    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
 ...
 * 发送红包接口
 */
class Hongbao_pub extends Wxpay_client_pub
{
	...
	function createXml()
	{
		try
		{
			...
			if($this->parameters["mch_billno"] == null) {
				throw new SDKRuntimeException("红包接口中，必须填写参数mch_billno！"."<br>");
			}elseif($this->parameters["nick_name"] == null){
				throw new SDKRuntimeException("红包接口中，缺少必填参数nick_name！"."<br>");
			...
			$this->parameters["send_name"]=$this->parameters["nick_name"];
			$this->parameters["min_value"]=$this->parameters["max_value"]=$this->parameters["total_amount"];
 ...
 * 发送红包接口
 */
class GroupHongbao_pub extends Wxpay_client_pub
{
	...
	function createXml()
	{
		try
		{
			...
			// $this->parameters["min_value"]=$this->parameters["max_value"]=$this->parameters["total_amount"];
			// $this->parameters["total_num"]=1;
			$this->parameters["amt_type"] = 'ALL_RAND';
		   	$this->parameters["wxappid"] = WxPayConf_pub::$APPID;//公众账号ID
		   	$this->parameters["mch_id"] = WxPayConf_pub::$MCHID;//商户号
		    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
		    $this->parameters["sign"] = $this->getSign($this->parameters);//签名
 ...
 * 企业转账接口
 */
class ZhuanZhang_pub extends Wxpay_client_pub
{
	...
	function createXml()
	{
		try
		{
			...
			if($this->parameters["partner_trade_no"] == null) {
				throw new SDKRuntimeException("接口中，必须填写参数partner_trade_no！"."<br>");
			}elseif($this->parameters["openid"] == null){
				throw new SDKRuntimeException("接口中，缺少必填参数openid！"."<br>");
			}elseif($this->parameters["amount"] == null){
				throw new SDKRuntimeException("接口中，缺少必填参数amount！"."<br>");
			}elseif($this->parameters["desc"] == null){
				throw new SDKRuntimeException("接口中，缺少必填参数desc！"."<br>");
			}elseif($this->parameters["spbill_create_ip"] == null){
				throw new SDKRuntimeException("接口中，缺少必填参数spbill_create_ip！"."<br>");
			}
...
class Wxpay_server_pub extends Common_util_pub 
{
	...
	function checkSign($out_name='WX')
	{
		$tmpData = $this->data;
		unset($tmpData['sign']);
		$sign = $this->getSign($tmpData,$out_name);//本地签名
